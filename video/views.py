...
class VideoListView(rest_generics.ListAPIView):
    """
    View to list videos ordered by publishing date
    """
...
class VideoSearchView(rest_generics.ListAPIView):
    """
    View to list down videos on the basis of search query
    """
    ...
    def get_queryset(self):
        ...
        title__exact_match_queryset = (
            Q(title__iexact=query) if query else Q()
        )  # Exact case insenstive title match
        description__exact_match_queryset = (
            Q(description__iexact=query) if query else Q()
        )  # Exact case insenstive  description match
        ...
        queryset = (
            queryset.filter(
                title__exact_match_queryset
                | description__exact_match_queryset
                | title_conatins_queryset
                | description_contains_queryset
            )
            .annotate(
